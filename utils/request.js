...
var buildAuthHeader = function buildAuthHeader(session) {
    ...
    if (session) {
        ...
        header['sessionKey'] = session;
    }

    return header;
};

var buildAuthParam = function buildAuthHeader(session) {
    var params = {};
...
function request(options) {
    ...
    function doRequest() {
        ...
        var authParam = buildAuthParam(Session.get());
        wx.request(utils.extend({}, options, {
            header: utils.extend({}, originHeader, authHeader),
            data: utils.extend({}, originParam, authParam),
            success: function (response) {
               
                var data = response.data;
                var error, message;
                if (data && data.resultCode === -1 && data.resultMessage ==='微信未授权') {
                    ...
                    callFail(error);
                    return;
                }else if(data && data.resultCode === -1 && data.resultMessage ==='请重新登录'){
